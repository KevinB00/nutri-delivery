...
const Post = (props) => {
  ...
  useEffect(() => {
    const fetchIsFavorited = async () => {
      ...
      const response = await fetch(
        `https://nutri-delivery-git-modificar-conexiondb-kevinb00s-projects.vercel.app/backend/actions/read/checkFavorito.php?id_post=${id_post}&id_usuario=${id_usuario}`
      );
    ...
    const fetchImgagePost = async () => {
      const id_post = props.postId;
      const response = await fetch(
        `https://nutri-delivery-git-modificar-conexiondb-kevinb00s-projects.vercel.app/backend/actions/read/getImagePost.php?id_post=${id_post}`
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const handleFavoriteClick = async () => {
    ...
    const response = await fetch(
      "https://nutri-delivery-git-modificar-conexiondb-kevinb00s-projects.vercel.app/backend/actions/update/updateFavorite.php",
      {
        method: "POST",
        headers: {
  ...
  const toggleComments = async () => {
    if (!showComments) {
      try {
        const response = await fetch(
          `https://nutri-delivery-git-modificar-conexiondb-kevinb00s-projects.vercel.app/backend/actions/read/getComentariosByPost.php?id_post=${props.postId}`
        );
        if (response.ok) {
          const data = await response.json();
          setComentarios(data);
  ...
  const handleSubmitComment = async () => {
    ...
    const response = await fetch(
      "https://nutri-delivery-git-modificar-conexiondb-kevinb00s-projects.vercel.app/backend/actions/create/addComentario.php",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <Card className="mt-3 mb-3 card-post">
      <Card.Body>
        ...
        {postImage ? <Card.Img className="image-post" variant="top" src={`https://nutri-delivery-git-modificar-conexiondb-kevinb00s-projects.vercel.app/backend/actions/create/${postImage}`} style={{height: "250px", width: "350px"}} /> : <div></div>}
